...
if [[ -n ${ANDROID_ARCHITECTURES} ]]; then
  ...
  if [[ ${SKIP_ffmpeg_kit} -ne 1 ]]; then
    if [ "$(is_darwin_arm64)" == "1" ]; then
       arch -x86_64 "${ANDROID_NDK_ROOT}"/ndk-build APP_STL=c++_shared -B 1>>"${BASEDIR}"/build.log 2>&1
    else
      "${ANDROID_NDK_ROOT}"/ndk-build APP_STL=c++_shared -B 1>>"${BASEDIR}"/build.log 2>&1
    fi
